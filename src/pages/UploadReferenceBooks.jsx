...
const UploadReferenceBooks = () => {
  ...
  const handleFileSelect = (files) => {
    ...
    const validFiles = fileArray.filter(file => {
      ...
      const isValidType = ['application/pdf', 'image/jpeg', 'image/png', 'image/jpg', 'application/epub+zip', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'].includes(file.type);
      ...
      if (!isValidType) {
        alert(`File ${file.name} is not a supported format. Please upload PDF, image, EPUB, or Word document files.`);
        return false;
      }
      return true;
    });
  ...
  const getFileIcon = (fileType) => {
    ...
    if (fileType.includes('epub')) return <FaBook className="text-orange-500" />;
  ...
  const handleUpload = async () => {
    ...
    try {
      for (const fileData of selectedFiles) {
        ...
        const fileId = `refbooks_${subject}_${Date.now()}_${Math.random()}`;
        let fileObj;
      ...
      const existingFiles = JSON.parse(localStorage.getItem(`refbooks_${subject}`) || '[]');
      const updatedFiles = [...existingFiles, ...uploadedFiles];
      
      try {
        localStorage.setItem(`refbooks_${subject}`, JSON.stringify(updatedFiles));
      } catch (storageError) {
      ...
      alert(`Successfully uploaded ${uploadedFiles.length} reference book(s)!`);
      setSelectedFiles([]);
      setUploadProgress({});
        ...
        <main className="main-content">
          <div className="dashboard-container">
            ...
            >
              ...
              >
                Upload Reference Books - {subjectDisplayName} 📚
              </motion.div>
              ...
              >
                Upload textbooks, reference materials, and study guides for {subjectDisplayName}
              </motion.div>
            </motion.div>
            ...
            >
              ...
              >
                ...
                <div className="upload-title">
                  Drop your reference books here
                </div>
                <div className="upload-subtitle">
                  or click to browse files (PDF, JPG, PNG, EPUB, Word - Max 100MB per file)
                </div>
                ...
                <input
                  ...
                  type="file"
                  multiple
                  accept=".pdf,.jpg,.jpeg,.png,.epub,.doc,.docx"
            ...
            {selectedFiles.length === 0 && (
              ...
              >
                <button
                  onClick={() => navigate(`/view-reference-books/${subject}`)}
...
export default UploadReferenceBooks;
