...
const UploadSchedule = () => {
  ...
  const handleFileSelect = (files) => {
    ...
    const validFiles = fileArray.filter(file => {
      ...
      const isValidType = ['application/pdf', 'image/jpeg', 'image/png', 'image/jpg', 'text/csv', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'].includes(file.type);
      ...
      if (!isValidType) {
        alert(`File ${file.name} is not a supported format. Please upload PDF, image, CSV, or Excel files.`);
        return false;
      }
      return true;
    });
  ...
  const getFileIcon = (fileType) => {
    ...
    if (fileType.includes('csv') || fileType.includes('excel') || fileType.includes('sheet')) return <FaFileAlt className="text-blue-500" />;
  ...
  const handleUpload = async () => {
    ...
    try {
      for (const fileData of selectedFiles) {
        ...
        const fileId = `schedule_${Date.now()}_${Math.random()}`;
        let fileObj;
      ...
      const existingFiles = JSON.parse(localStorage.getItem('lecture_schedules') || '[]');
      ...
      alert(`Successfully uploaded ${uploadedFiles.length} schedule file(s)!`);
      setSelectedFiles([]);
      setUploadProgress({});
        ...
        <main className="main-content">
          <div className="dashboard-container">
            ...
            >
              ...
              >
                Upload and manage lecture schedules, timetables, and class timings
              </motion.div>
            </motion.div>
            ...
            >
              ...
              >
                ...
                <div className="upload-title">
                  Drop your lecture schedules here
                </div>
                <div className="upload-subtitle">
                  or click to browse files (PDF, JPG, PNG, CSV, Excel - Max 100MB per file)
                </div>
