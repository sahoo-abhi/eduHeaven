...
const ViewPapers = () => {
  ...
  const fetchFiles = async () => {
    ...
    try {
      ...
      const storedFiles = localStorage.getItem(`papers_${subject}_${paperType}`);
      const files = storedFiles ? JSON.parse(storedFiles) : [];
      
      // Show all files for the subject and paper type (no user filtering)
  ...
  const formatPaperType = (paperType) => {
    return paperType === 'mid-sem' ? 'Mid Semester' : 'End Semester';
  };
  ...
  const handleBackToSubject = () => {
    navigate(`/question-papers/${subject}`);
  };
            ...
            >
              ...
              >
                <span className="bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent">
                  {formatPaperType(paperType)} Papers 📄
                </span>
              </motion.div>
              ...
              >
                {formatSubjectName(subject)} - {formatPaperType(paperType)} Question Papers
              </motion.div>
            </motion.div>
            ...
            >
              {loading ? (
                <div className="flex items-center justify-center py-12">
                  ...
                  <span className="ml-3 text-white/80">Loading papers...</span>
                </div>
              ) : files.length === 0 ? (
                <motion.div 
                ...
                >
                  <div className="dashboard-card p-8">
                    ...
                    <p className="text-white/60 mb-6">
                      No {formatPaperType(paperType).toLowerCase()} papers have been uploaded for {formatSubjectName(subject)} yet.
                    </p>
                    <motion.button
